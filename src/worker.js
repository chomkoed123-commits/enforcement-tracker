...
function genSalt()  { return bytesToHex(crypto.getRandomValues(new Uint8Array(16))); }
...
async function initDb(sql) {
  await sql`CREATE TABLE IF NOT EXISTS users (
    ...
    password_hash TEXT NOT NULL, salt TEXT NOT NULL, role TEXT DEFAULT 'user',
  ...
  await sql`CREATE TABLE IF NOT EXISTS action_logs (
    id SERIAL PRIMARY KEY,
    case_id TEXT REFERENCES enforcement_cases(id) ON DELETE CASCADE,
    changed_by TEXT, user_id INTEGER REFERENCES users(id) ON DELETE SET NULL,
  ...
  return { ok: true, message: "Tables ready. Default admin: admin / Admin@1234" };
}

// ── LOGIN / AUTH ──────────────────────────────────────────────────────────────
async function login(sql, body, request) {
  ...
  await sql`INSERT INTO user_sessions (user_id, token, ip_address, user_agent) VALUES (${user.id}, ${token}, ${ip}, ${ua})`;
  return { ok: true, token, user: { id: user.id, username: user.username, display_name: user.display_name, role: user.role, must_change_password: user.must_change_password } };
}
...
async function toggleUser(sql, userId, action) {
  if (action === 'unlock') {
    ...
    const salt = genSalt();
    const hash = await hashPassword("Reset@1234", salt);
...
async function getCases(sql, qs) {
  ...
  const cases = await sql(`SELECT * FROM enforcement_cases ${where} ORDER BY priority DESC, amount DESC ${limitClause}`, params);
...
async function bulkImport(sql, body, session) {
  ...
  if (count === 0) return { ok: false, error: `นำเข้าไม่สำเร็จ (${errorCount} แถวมีปัญหา)` };
  if (firstId) {
    try {
...
async function getOnlineUsers(sql) {
  return await sql`
    SELECT DISTINCT ON (s.user_id) u.display_name, u.role, s.last_seen, s.ip_address, s.created_at AS session_start
    FROM user_sessions s JOIN users u ON u.id = s.user_id
    WHERE s.last_seen > NOW() - INTERVAL '15 minutes' AND s.expires_at > NOW()
    ORDER BY s.user_id, s.last_seen DESC
  `;
}

// ── API ROUTER ────────────────────────────────────────────────────────────────
async function handleApi(request, env, path, qs, token, body) {
  ...
  try {
    ...
    if (method === "POST" && path === "/login") return resp(200, await login(sql, body, request));
    if (method === "POST" && path === "/seed") {
      const [cnt] = await sql`SELECT COUNT(*)::int as c FROM enforcement_cases`;
      if (cnt.c > 0) return resp(400, { error: `ข้อมูลมีอยู่แล้ว ${cnt.c} เคส` });
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname.startsWith('/api')) {
      if (!env.DATABASE_URL) return resp(500, { error: "DATABASE_URL is not set" });
      const path = url.pathname.replace(/^\/api/, '') || '/';
      const qs = Object.fromEntries(url.searchParams);
      const token = request.headers.get('x-session-token') || '';
      let body = {};
      try { const t = await request.text(); if (t) body = JSON.parse(t); } catch(_) {}
      return handleApi(request, env, path, qs, token, body);
    }
